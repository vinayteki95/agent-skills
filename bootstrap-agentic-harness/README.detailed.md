# bootstrap-agentic-harness — detailed guide

Start with the [short README](./README.md) if you want the loop and usage only.

Turn a PRD, tech doc, or one-paragraph vision into a fully wired **agent-first repository**: a short `AGENTS.md` map, a `docs/` system of record, living exec plans, tiered quality sensors, git hooks, and CI.
...
Philosophy is distilled from [OpenAI Harness Engineering](https://openai.com/index/harness-engineering/): progressive disclosure, same-PR doc sync, and **failures encoded back into the harness**. Depth on *why* each file exists lives in [`reference.md`](./reference.md). The agent procedure lives in [`SKILL.md`](./SKILL.md).
...
6. [Folder structure this skill uses](#folder-structure-this-skill-uses)
...
16. [Changing the harness payload](#changing-the-harness-payload)
...
## Folder structure this skill uses

```
bootstrap-agentic-harness/             ← symlink this folder into ~/.cursor/skills/
├── README.md                          ← short flow + usage
...
What this skill fixes versus a naive copy of a mature harness is spelled out in [`reference.md`](./reference.md): index/folder drift, vestigial stack sensors, hardcoded scan paths, duplicated maintenance tables, no self-check, and project-specific coupling.
...
## Changing the harness payload

Edit files under `harness/`. Keep `{{TOKENS}}` for values `bootstrap.sh` substitutes. Keep `<!-- FILL: -->` for values the **agent** must write from a PRD. If you add a required file, update `harness/scripts/verify-harness.sh` and the inventory in `SKILL.md` / `reference.md` in the same change.

This skill lives in the [agent-skills](https://github.com/vinayteki95/agent-skills) collection. MIT — see [`../LICENSE`](../LICENSE).
